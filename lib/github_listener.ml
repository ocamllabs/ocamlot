...
let make_listener service_fn ~root ~host ~port t_resource =
  ...
  service_fn
    ...
    ~startup:(List.map (fun watch ->
      Persist.get ~access:`Private token_kp
      >>= begin function
      ...
      let url = Uri.resolve "http" base (Uri.of_string (notify_uriref watch)) in
      Github_hook.(
        connect github registry url (watch,notification_handler)
        >>= fun endpoint -> match endpoint with
          | {status=Indicated} ->
              eprintf "%s wedged prerequest\n%!" (github_error_str watch);
              return ()
          | {status=Pending} ->
